...
import { CounterfactualPanel, PlaybookPanel } from "@/components/revive/playbook";
import { DiagnosisPanel, RecoveryScorePanel } from "@/components/revive/diagnosis";
...
function IncidentDetail() {
  ...
  const { meta, incident, report, audit, actions, siblings } = result.data;
  ...
  // The safety rail only lights a stage that has demonstrably happened.
  const executed = actions.filter((a) => a.executionStatus === "executed");
  const anyBlocked = actions.some((a) => a.policyStatus === "blocked");
  const pipelineStage = executed.some((a) => a.verification)
    ? 5
    : executed.length > 0
      ? 4
  ...
  const pipelineNote =
    pipelineStage < 0
      ? "Detection has completed and is written to the audit ledger. No stage beyond detection has run for this incident — REVIVE holds no execution authority."
      : pipelineStage === 0
        ? "The AI has produced a diagnosis. Nothing has been proposed to the policy engine yet."
        : pipelineStage === 1
          ? "A bounded playbook has been proposed and evaluated. Nothing executes until the policy engine approves it."
          : pipelineStage === 2
            ? "The policy engine has cleared at least one action for a bounded canary batch."
            : "A bounded canary batch ran in test mode and its result was verified against the ledger. Every step is in the audit trail.";
  ...
  return (
    ...
    >
      ...
      <Panel className="mt-3">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <Metric
            ...
            hint={
              executed.length > 0
                ? "captured by verified canary batches"
                : "no recovery batch has run"
            }
        ...
        <div className="mt-4">
          ...
          <p className="mt-2 text-[11px] leading-relaxed text-muted-foreground">{pipelineNote}</p>
        </div>
      </Panel>
      ...
      <div className="mt-4 grid items-start gap-4 lg:grid-cols-3">
        ...
        <DiagnosisPanel incident={incident} narrative={narrative} className="lg:col-span-2" />
      </div>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <RecoveryScorePanel incident={incident} />
        <CounterfactualPanel
          atRiskPaise={incident.revenueAtRiskPaise}
          actions={actions}
        />
      </div>
      ...
      <PlaybookPanel
        ...
        actions={actions}
        investigated={incident.diagnosis !== null}
      />
